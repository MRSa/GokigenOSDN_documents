...
- [AirA01d : OLYMPUS AIR A01用のAndroidアプリケーション](#aira01d--olympus-air-a01用のandroidアプリケーション)
  ...
  - [撮影画像一覧画面](#撮影画像一覧画面)
    - [戻るボタン](#戻るボタン)
    - [ファイル数表示](#ファイル数表示)
    - [グリッド表示/リスト表示切替ボタン](#グリッド表示リスト表示切替ボタン)
    - [一覧フィルタボタン](#一覧フィルタボタン)
    - [リロードボタン](#リロードボタン)
  - [画像表示](#画像表示)
  - [カメラプロパティ設定画面](#カメラプロパティ設定画面)
  - [アプリ設定画面](#アプリ設定画面)
    - [カメラに自動接続](#カメラに自動接続)
    - [コマンド同時送信数の削減](#コマンド同時送信数の削減)
    - [操作説明](#操作説明)
...
以下にAirA01dの画面遷移を示します。

![画面遷移](images/screen_transition.png)

起動直後は、撮影画面を表示します。撮影画面からは、撮影画像一覧画面、カメラプロパティ設定画面、アプリ設定画面に移動可能です。
なお、アプリ設定画面以外への移動は、カメラと接続している状態でのみ可能です。

撮影画像一覧画面は、グリッド表示とリスト表示の切り替えができます。
また、画像をタッチすると、タッチした画像を大きく表示でき、その状態から画像を取得することができます。
...
アプリを起動すると、Wi-Fi経由でカメラの接続を試行します。（この試行は、アプリ設定画面にあるスイッチで無効にできます。）
...
撮影プレビューモードがONの時、直前の撮影画像を表示します。タッチすると、撮影画像一覧画面に遷移します。
長押しをすると、直前の画像を再取得し、ボタン上に撮影画像を表示します。
**未撮影のタイミングで、本ボタンの長押しは避けてください。** カメラ側で通信が詰まる可能性があります。
...
カメラ内に格納されているカメラ画像の一覧表示を行います。

![撮影画像一覧画面](images/picture_list.png)

グリッド表示時は、動画ファイルとRAWファイルには、右上にアイコンを表示します。

![ファイルタイプ表示(グリッド表示時)](images/picture_file_type.png)

リスト表示時には、画像とファイル名、撮影日時およびファイルサイズを表示します。

![リスト表示時のファイル情報](images/picture_list_item.png)

撮影画像一覧画面で、サムネイル画像の取得に失敗することがあります。この場合は画像取得に失敗した画像をタッチする、あるいは画面外に一度スクロールして元に戻すことで再取得します。

![サムネイル画像取得失敗](images/download_failure.png)
...
ファイル数を表示します。「現在一覧表示しているファイル数 / カメラ内に格納されたファイル数」を表示します。

### グリッド表示/リスト表示切替ボタン

ボタンを押すと、画像一覧をグリッドで表示するか、リストで表示するか切り替えることができます。
画面オープン時の初期状態はグリッド表示です。

![グリッド表示/リスト表示の切り替え](images/picture_list_type.png)

### 一覧フィルタボタン

画像一覧で、一覧フィルタ設定パネルを表示・非表示を切り替えるためのボタンです。

![一覧フィルタ設定パネル](images/panel_filtering.png)

一覧フィルタ設定パネルでは、以下の設定が可能です。

- 画像の並び順： 新しい順/古い順
- 表示開始日・表示終了日
- ファイル種類（全て/JPEG/RAW/動画/その他）

### リロードボタン

本ボタンで画像一覧の情報を再取得することができます。（基本、使用しなくても大丈夫です。）

## 画像表示

一覧画面で画像をタッチすると、タッチした画像を画面いっぱいに表示します。左右スワイプ操作で前後の画像に移ることができます。

![画像表示](images/detail_picture.png)

右上の「×」を押すと、一覧表示に戻ります。隣のダウンロードボタンを押すと、画像のダウンロードができます。

![画像のダウンロード](images/download_picture.png)

ダウンロードボタンを押すと、画像サイズを選択するダイアログを表示します。JPEGファイルのときのみ、ダウンロードサイズを選べます。
右下の「開始」ボタンを押すと画像のダウンロードを開始します。

ダウンロードした画像は **/DCIM/AirA01d/** に **オリジナルファイル名_ダウンロード年月日時分秒.拡張子** という名前で保存します。(例えば、PB032295.JPG を取得した場合、PB032295_20260618221010.JPG というようなファイル名になります。)
...
- **アートフィルター**
  - アートフィルター (Popart)
  - アートフィルター (DayDream)
  - アートフィルター (Rough Monochrome)
  - アートフィルター (Toy Photo)
  - アートフィルター (Miniature)
  - アートフィルター (Cross Process)
  - アートフィルター (Dramatic Tone)
  - アートフィルター (Ligne Clair)
  - アートフィルター (Pastel)
  - アートフィルター (Vintage)
  - アートフィルター (Partcolor)
  - パートカラー用 色相
...
カメラプロパティの詳細は、[AIR A01のカメラプロパティ](https://github.com/MRSa/GokigenOSDN_documents/blob/main/Applications/AirA01b/opc-camera_properties.md)を参考にしてください。
...
デフォルトはOFFのスイッチです。カメラへのコマンド送信を極力減らし、カメラ側でのコマンド詰まり発生を抑えることを目的としています。
ただ、あまり効果はないかもしれません。気休め程度と考えてください。
...
（軽減策として、アプリ設定画面に、スイッチ「コマンド同時送信数の削減」を用意していますが、完全ではありません。ご了承下さい。）
...
- 1.5.0 : カメラ撮影画像一覧画面を搭載し、カメラから画像を取得する機能を搭載
